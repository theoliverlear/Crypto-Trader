...
@Service
class RsaKeyService(
    @param:Value("\${security.jwt.rsa.public-key-pem:}") private val publicPem: String?,
    ...
    init {
        if (this.isValidKeys(this.publicPem, this.privatePem)) {
            val publicKeyBytes: ByteArray = Base64.getDecoder().decode(this.stripPemHeaders(this.publicPem))
            val privateKeyBytes: ByteArray = Base64.getDecoder().decode(this.stripPemHeaders(this.privatePem))
